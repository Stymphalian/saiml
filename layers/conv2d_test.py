...
class TestConvs(unittest.TestCase):
    ...
    @unittest.skip("Vectorized version does not work")
    def test_conv2d_forward_with_stride_and_padding_vectorized(self):
        layer = Conv2d(
            input_shape=(1,5,5),
            num_kernels=1,
            kernel_size=3,
        ...
        dEdX = X.grad.value()
        dEdW = layer.W.grad.value()
        dEdb = layer.b.grad.value()
