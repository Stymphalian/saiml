...
class OperatorsTest(unittest.TestCase):
    ...
    def test_add(self):
        x1 = ag.Tensor(np.arange(3, dtype=np.float64) + 1.0)
        x2 = ag.Tensor(np.arange(3, dtype=np.float64) + 2.0)
        def forward(params):
    ...
    def test_matmul(self):
        x1 = ag.Tensor(np.arange(9, dtype=np.float64).reshape(3,3) + 1.0)
        x2 = ag.Tensor(np.arange(3, dtype=np.float64).reshape(3,1) + 1.0)
        def forward(params):
            self.unravel_params(params, x1, x2)
    ...
    def test_sin(self):
        x = ag.Tensor(np.arange(9, dtype=np.float64) + 1.0)
        def forward(params):
    ...
    def test_sum(self):        
        x = ag.Tensor(np.arange(3, dtype=np.float64) + 1.0)
        def forward(params):
            self.unravel_params(params, x)
            return ag.sum(x)
        self.numeric_check(forward, x)
    ...
    def test_softmax(self):
        ...
        def forward(params):
            x_params = params[:x.data.size].reshape(x.data.shape)
            x.data = x_params
            z1 = ag.softmax(x)
            return ag.sum(z1).value()
        
        params = np.concatenate([x.value().reshape(-1)])
        predictedGradients = np.concatenate([x.grad.reshape(-1)])
        grads, diff = ag.utils.numeric_gradient_check(
            forward, params, predictedGradients)
